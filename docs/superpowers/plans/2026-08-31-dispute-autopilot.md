...
Half the sample is deliberately degraded (required evidence removed) to measure
the refusal rate, which is the defense-only guarantee under test.
"""
import json
from pathlib import Path
...
def main(n: int = N_CASES) -> dict:
    ...
    grounded_scores, refusals, complete_cases = [], 0, 0
